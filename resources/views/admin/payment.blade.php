        ...
        <div class="bg-white p-6 rounded-lg shadow-lg">
            ...
            <div class="flex justify-between mb-4">
                <div class="flex space-x-4">
                    <form method="GET" action="{{ route('admin.payment.form') }}" class="flex items-center">
                ...
                <!-- Search Form -->
                <form action="{{ route('admin.payment.form') }}" method="GET" class="flex items-center">
                    ...
                    <button type="submit" class="px-4 py-2 bg-[#1A1363] text-white rounded-r-lg hover:bg-[#160f70] focus:outline-none">
                        Search
                                ...
                                <!-- Add this after your table or where you want the print button to appear -->
                <div class="mt-6 flex justify-end">
                    <a href="{{ route('admin.payment-pdf.download', ['date' => $selectedDate]) }}" 
                    class="px-4 py-2 bg-[#1A1363] text-white rounded-lg shadow-md hover:bg-[#0f0c5c] flex items-center">
                        <i class="fas fa-print text-xs mr-1"></i>
                        <span class="text-sm">Print</span>
                    </a>
                </div>
                <!-- Sorting Dropdown -->
                <form method="GET" action="{{ route('admin.payment.form') }}" class="flex items-center ml-4">
                    <input type="hidden" name="date" value="{{ $selectedDate }}"> <!-- Preserve the selected date -->
            ...
            <div id="edit-popup" class="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-75 hidden">
                <div class="bg-white p-6 rounded-lg shadow-lg max-w-lg w-full">
                    ...
                    <form id="edit-form" action="{{ route('admin.payment.update', '') }}" method="POST">
                        @csrf
                        @method('PUT')
            ...
            <script>
                function clearSort() {
                    window.location.href = "{{ route('admin.payment.form') }}";
                }
                ...
                function openEditPopup(id, memberName, subscriptionPlan, promo, amount, datePaid) {
                    ...
                    document.getElementById('edit-form').action = `{{ route('admin.payment.update', ':id') }}`.replace(':id', id); // Set the form action URL
